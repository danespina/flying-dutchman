document.addEventListener("DOMContentLoaded", () => {
  const canvas = document.getElementById("theSea");
  const ctx = canvas.getContext("2d");
  const background = new Image();
  const dutchman = new Image();
  const ghost = new Image();
  const water = new Image();
  background.onload = () => {};
  dutchman.onload = () => {};
  ghost.onload = () => {};
  water.onload = () => {};
  background.src = "../assets/spooky_background.png";
  dutchman.src = "../assets/flying_dutchman.png";
  ghost.src = "../assets/ghost.png";
  water.src = "../assets/water.png";

  const radius = 50;
  let x = canvas.width - radius;
  let y = canvas.height / 2;
  let dx = 5;
  let boatY = 3 * radius;
  let boatX = canvas.width / 10;
  let dBoatY = 0;
  let dBoatX = 0;
  let spacePressed = false;
  let aPressed = false;
  let dPressed = false;
  let boatsJumped = -1;
  let t = 0;
  let seaLevel = [];
  for (var i = 0; i < 900; i++) {
    seaLevel[i] = radius / 2 * Math.sin((1 / 30)* i) + canvas.height * 0.75;
  }
  const gravity = radius / 10;
  let gravitySpeed = 0;
  let jumpSize = 1;
  const jumpAccel = 1 * radius;
  let jumping = false;

  const jump = () => {
    jumping = true;
    if ((boatY - (canvas.height - seaLevel[canvas.width / 10] + radius)) < 1) {
      dBoatY -= jumpAccel;
    }
  }

  const draw = () => {
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    // ctx.drawImage(water, 0, 400, 900, 400);
    ctx.drawImage(background, 0, 0, canvas.width, canvas.height);

    if ((x + dx) > (canvas.width - radius) || x + dx < 0) {
      dx = -dx;
      // boatsJumped++;
    }
    if (((x > boatX && x < (boatX + radius)) || (x + radius > boatX && x + radius < boatX + radius)) &&
    ((y < (canvas.height - boatY + radius) && y > (canvas.height - boatY)) ||
  (y + radius < canvas.height - boatY + radius) && (y + radius > (canvas.height - boatY)))) {
      dx = 0;
      dBoatX = 0;
      // document.location.reload();
      // alert(`you collected ${boatsJumped} souls!`);
    }
    if (Math.abs(x - boatX) < radius / 2) {
      boatsJumped++;
    }
    if (spacePressed && !jumping) {
      jump();
      // dBoatY = jumpSize;
      // boatY += dBoatY;
    } else {
      dBoatY += gravity;
      boatY -= dBoatY;
      boatY = Math.max(canvas.height - seaLevel[boatX] + radius, boatY);

      if ((boatY - (canvas.height - seaLevel[boatX] + radius)) < radius / 2 ) {
        dBoatY = 0;
        jumping = false;
      }

      if (aPressed && boatX > 10) {
        dBoatX = -10;
        boatX += dBoatX;
      } else if (dPressed && boatX + radius < 890) {
        dBoatX = 10;
        boatX += dBoatX;
      }
    }
    ctx.beginPath();
    ctx.fillStyle = "#FFFFFF";
    ctx.fillText(`${boatsJumped}`, 100, 100);
    ctx.closePath();


    drawWater();
    drawWave();
    drawBoat();

    x += dx;
    y = seaLevel[x] - radius;
  }

  const drawWater = () => {
    ctx.beginPath();
    ctx.moveTo(0, canvas.height * 0.75);
    for (var i = 0; i < 900; i++) {
      seaLevel[i] = radius / 2 * Math.sin((1 / 30)* (i + t)) + canvas.height * 0.75;
      ctx.lineTo(i, seaLevel[i]);
    }
    // ctx.lineTo(canvas.width, 0);
    // ctx.lineTo(0, 0);
    // ctx.lineTo(0, canvas.height * 0.75);
    ctx.lineTo(canvas.width, canvas.height);
    ctx.lineTo(0, canvas.height);
    ctx.lineTo(0, canvas.height * 0.75);
    t++;
    // ctx.clip();
    ctx.fillStyle = "rgba(0,0,255,1)";
    ctx.fill();
    ctx.closePath();

  }

  const drawWave = () => {
    ctx.beginPath();
    ctx.rect(x, y, radius, radius);
    ctx.fillStyle = "#00FF00";
    ctx.fill();
    ctx.closePath();
    ctx.drawImage(ghost, x, y, radius, radius);
  }

  const drawBoat = () => {
    ctx.beginPath();
    ctx.rect(boatX, canvas.height - (boatY), radius, radius);
    ctx.fillStyle = "#FF0000";
    ctx.fill();
    ctx.closePath();
    ctx.drawImage(dutchman, boatX, canvas.height - (boatY), radius, radius);
  }


  const keyDownHandler = (e) => {
    if (e.keyCode === 32) {
      spacePressed = true;
    } else if (e.keyCode === 65) {
      aPressed = true;
    } else if (e.keyCode === 68) {
      dPressed = true;
    }
  }

  const keyUpHandler = (e) => {
    if (e.keyCode === 32) {
      spacePressed = false;
    } else if (e.keyCode === 65) {
      aPressed = false;
    } else if (e.keyCode === 68) {
      dPressed = false;
    }
  }

  document.addEventListener("keydown", keyDownHandler, false);
  document.addEventListener("keyup", keyUpHandler, false);

  setInterval(draw, 50);
})
